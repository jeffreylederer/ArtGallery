...
namespace ArtGallery
{
    ...
    public static class PictureDL
    {
        ...
        public static ArtGalleryDS.OrphanPictureDataTable GetOrphanPicture()
        {
            ...
            // look at all the book covers
            ArtGalleryDS.BookDataTable table1 = BookDL.Get();
            foreach (string file in files)
            {
                FileInfo fi = new FileInfo(file);
                string filename = fi.Name;
                if (filename == "Missing.jpg" || filename == "artgallery.p12" || filename == "paypal_cert_pem.txt")
                    continue;
                var query = from p in table1 where filename.ToUpper() == p.PicturePath.ToUpper() select p.PicturePath;
                if (query.Count() == 0)
                {
                    object[] missingfile = new object[1];
                    missingfile[0] = filename;
            ...
            // look at all the pictures in inside the book
            ArtGalleryDS.BookPageDataTable table2 = BookPageDL.Get();
            foreach (string file in files)
            {
                FileInfo fi = new FileInfo(file);
                string filename = fi.Name;
                if (filename == "Missing.jpg" || filename == "artgallery.p12" || filename == "paypal_cert_pem.txt")
                    continue;
                var query = from p in table2 where filename.ToUpper() == p.PicturePath.ToUpper() select p.PicturePath;
                if (query.Count() == 0)
                {
                    object[] missingfile = new object[1];
                    missingfile[0] = filename;
